...
class Encoder(nn.Module):
    def __init__(self, 
        ...
        self.device = device
        self.perception = lenet.make_model(16)
        self.perception.load_state_dict(torch.load('./model.pth.tar_66.6_match'))
        ...
        self.sym_embedding = nn.Embedding(16, hid_dim)
    ...
    def forward(self, src, src_mask):
        ...
        src = src.reshape((-1, src.shape[-3], src.shape[-2], src.shape[-1]))
        src = self.dropout((self.perception(src) * self.scale))
        src = src.reshape((batch_size, src_len, -1))
        src = F.softmax(src, dim=-1)
        #src = [batch size, src len, symbol categories]
        indices = torch.arange(0, 16).unsqueeze(0).repeat(batch_size, 1).to(self.device)
        embed = self.sym_embedding(indices)
        #embed = [batch_size, symbol categories, hid_dim]
        src = torch.bmm(src, embed)
...
def train(model, dataloader, optimizer, criterion, clip):
    ...
    for sample in tqdm(dataloader):
        ...
        src_mask = sample['mask']
        src = src.to(device)
        src_mask = src_mask.to(device)
        trg = trg.to(device)
        optimizer.zero_grad()
        
        output = model(src, trg[:,:-1], src_mask)
