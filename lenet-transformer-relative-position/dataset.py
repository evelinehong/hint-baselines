...
from utils import ROOT_DIR, IMG_DIR, IMG_TRANSFORM, pad_image
...
class HINT(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_list = [SYM2ID('s')] + label_list
...
def HINT_collate(batch):
    ...
    max_len_o = np.max([len(str(x['res'])) for x in batch])
